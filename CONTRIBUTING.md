# Contributing to Bolt Spring Boot Starter

Thanks for your interest in contributing to Bolt Spring Boot Starter! We welcome pull requests, issues, and feedback from the community.
...
- Version of the starter and relevant environment details (Java version, Spring Boot version)
...
- `bolt-spring-boot-annotations/` - Annotation definitions (`@SlackController`, `@SlashCommand`, etc.)
- `bolt-spring-boot-autoconfigure/` - Auto-configuration, handler scanning, servlet registration
- `bolt-spring-boot-starter/` - Dependency aggregator (what consumers add to their project)

## Testing

We maintain high test coverage. When adding new features, please include:
- Unit tests for core logic
- Integration tests using `WebApplicationContextRunner` for auto-configuration behavior
...
- `feat: add support for new Bolt handler type`
- `fix: handle proxy beans in annotation scanner`
- `docs: add error handling examples to README`
- `test: add integration tests for custom servlet paths`
- `refactor: extract shared handler registration logic`
